...
App.initLeafletMarker = function() {
    ...
    App.st.marker.on('dragend', function() {
        let newLatLng = App.st.marker.getLatLng();
...
App.initFreeDraw = function(data) {
    ...
    App.st.map.on("draw:created", function (event) {
        ...
        let json = App.st.drawnItems.toGeoJSON();
        if ("features" in json && json.features.length && "geometry" in json.features[0]) {
            ...
            let latLng = App.st.drawnItems.getBounds().getCenter();
    ...
    if (o.address) {
        ...
        if ($('[name="city_name"]').length) {
            ...
            if ($('[name="city_id"]').length) {
                let selects = $('[name="city_id"]').select2('data');
...
App.showOtherAreas = function(items) {
    ...
    items.map((item) => {
        if (item.area.length) {
            let polygon = L.polygon(JSON.parse(item.area));
            polygon.setStyle({fillColor: '#f3aa17', fillOpacity: 0.1, color: '#f3aa17', opacity: 0.25});
            polygon.addTo(App.st.otherItems);
            polygon.bringToBack();
            polygon.bindPopup(`#${item.id}. ${item.name}`);
        }
    })
